...
export default function ActivateAccountPage() {
  ...
  useEffect(() => {
        const activateAccount = async () => {
          try {
            ...
            return  String("Votre lien d'activation a expiré ou le compte a été activé.");
          }
        };
  ...
  return (
    <div className="h-screen w-screen flex flex-col justify-center items-center  relative">
        {loading ? <CircleAnimationIcon/> : <div
            className="z-10 flex flex-col items-center justify-start space-y-8 max-w-md border border-gray-300 py-3 px-4">
            {/* Logo */}
            <img src={LogoBun} className="w-16 h-auto" alt="Logo Bun"/>
            <p className="text-sm md:text-base capitalize font-medium">activez votre compte</p>
...
function ActivationAccountMessage({message}) {
    return (
        <div className="max-w-md text-center">
           <p className="text-base md:text-base font-medium text-red-500">{message}</p>
        </div>
    );
}
