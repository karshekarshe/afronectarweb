...
import "../App.css";
import {CaddyIcon} from "./Products";
...
export default function ProductsGrid({product}) {
  ...
  useEffect(() => {
      ...
      setVariants(product[0]?.variants)
  }, [product]);
      ...
      <section className="py-5">
        ...
        <div className="container mx-auto px-2 md:px-6  grid lg:grid-cols-4 md:grid-cols-3 grid-cols-2 gap-2  md:gap-6 py-10">
          {variants && variants.map((variant, index) => {
            return (
                <div class="product_item relative grid gap-2 py-2 justify-center text-center transition-[0.5s] hover:bg-amber-100 bg-amber-50">
                    <div className="flex flex-row items-center justify-between py-2 px-2 w-full gap-2">
                                <span className="inline-flex items-center gap-2 bg-orange-100 text-orange-500 font-semibold px-2 py-2 rounded-2xl text-xs md:text-sm">
                                ...
                                <button className="inline-flex items-center px-2 py-2 text-xs md:text-sm  bg-white rounded-full gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                         strokeWidth={1.5}
                    ...
                    <img
                        src={variant.img}
                        class="mx-auto max-w-[150px] md:max-w-[180px] h-auto"
                        alt="product_img"
                    />
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-row items-center justify-between gap-2 w-full">
                            ...
                            <a
                                href={`http://localhost:3000/products/${variant.slug}`}
